-- ============================================================
-- 016_fix_radius_default.sql
-- The COALESCE fallback in 015 was 40.2km (25mi), which meant
-- any user whose discover_radius_km hadn't been written yet
-- (everyone who hadn't opened the app post-migration) was
-- treated as having a 25mi radius and became invisible to
-- anyone further away.
-- Fix: default the fallback to 482.8km (300mi = our max), so
-- unset users are permissive. Once a user opens the app their
-- real radius is stored and takes over.
-- Also bumps the column default so new rows start at max radius.
-- ============================================================

ALTER TABLE user_profiles
  ALTER COLUMN discover_radius_km SET DEFAULT 482.8;

-- Backfill any existing rows still at the old 40.2336 default
UPDATE user_profiles
   SET discover_radius_km = 482.8
 WHERE discover_radius_km IS NULL
    OR discover_radius_km = 40.2336;

-- Replace the function with the corrected COALESCE fallback
